...
def myMHA(Q, K, V, nb_heads, mask=None, clip_value=None, score_bias=None):
    """
    ...
    if score_bias is not None:
        # score_bias должен быть размера (bsz, 1, nb_nodes) или броадкаститься
        if nb_heads > 1:
            # Повторяем bias для каждой головы
            bias_reshaped = torch.repeat_interleave(score_bias, repeats=nb_heads, dim=0)
            attn_weights = attn_weights + bias_reshaped
        else:
            attn_weights = attn_weights + score_bias
    ##################################################
