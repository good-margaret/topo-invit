...
def run_tsplib_test_knn(model,action_k,state_k,path):
    ...
    for i in range(len(tsplib_names)):
        ...
        with torch.no_grad():
            ##########################################################
            tour, _, _ = model(X, action_k, state_k, choice_deterministic=True)
...
def run_cvrplib_test_knn(model,action_k,state_k,path):
    ...
    for i in range(len(cvrplib_names)):
        ...
        with torch.no_grad():
            #################################################
            tour, _, _ = model(input_aug, action_k, state_k, capacity, problem_type, choice_deterministic=True)
    ...
    for distribution in distributions:
        for i in range(len(sizes)):
            ...
            for j in range(num):
                ...
                with torch.no_grad():
                    ###################################################################
                    tour, _, _ = model(X, local_k, global_k, choice_deterministic=True, if_use_local_mask=if_use_local_mask)
    ...
    for distribution in distributions:
        for i in range(len(sizes)):
            ...
            for j in range(num):
                ...
                with torch.no_grad():
                    #######################################################
                    tour, _, _ = model(input_aug, local_k, global_k, cap, problem_type, choice_deterministic=True, if_use_local_mask=if_use_local_mask)
