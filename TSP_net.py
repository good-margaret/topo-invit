...
class TSP_net(nn.Module): 
    ...
    def forward(self, x, action_k, state_k, choice_deterministic=False, if_use_local_mask = False):
        ...
        ### NEW: ГЕНЕРАЦИЯ ЭМБЕДДИНГОВ ДЛЯ RTD-LITE ###
        # Чтобы получить эмбеддинги для всех узлов сразу, мы используем первый state_encoder.
        # Мы создаем фиктивный KNN-индекс (каждый узел видит сам себя и соседей), 
        # чтобы получить стабильное представление каждого узла.
        
        # Для простоты и эффективности возьмем проекцию через первый энкодер или 
        # прогоним через него все узлы. Здесь мы берем результат первого слоя:
        all_nodes_idx = torch.arange(nb_nodes, device=x.device).repeat(bsz, 1)
        # Мы используем первый state_encoder для получения "статичных" признаков всех узлов
        # В качестве 'last_visited' и 'first_visited' используем среднее или первый узел
        dummy_node = x[:, 0:1, :] 
        
        # Получаем эмбеддинги всех узлов (используем k=nb_nodes или фиксированное значение)
        # Эти эмбеддинги будут возвращены для вычисления RTD Loss
        with torch.set_grad_enabled(self.training):
            # Мы вызываем первый энкодер для формирования "карты" признаков
            # Примечание: Мы берем только часть вывода, относящуюся к самим узлам
            initial_embeddings = self.state_encoders[0](x, all_nodes_idx, dummy_node, dummy_node)
            # Отсекаем служебные токены (last/first), оставляем только узлы [bsz, nb_nodes, dim_emb]
            node_embeddings = initial_embeddings[:, :nb_nodes, :]
        ###############################################
        ...
        return tours, sumLogProbOfActions, node_embeddings