...
class VRP_net(nn.Module): 
    ...
    def forward(self, x, action_k, state_k, capacity, problem = 'cvrp', choice_deterministic=False, if_use_local_mask = False):
        ...
        ### NEW: ГЕНЕРАЦИЯ ЭМБЕДДИНГОВ ДЛЯ VRP ###
        # Получаем признаки клиентов, учитывая их координаты и текущий спрос
        all_nodes_idx = torch.arange(nb_nodes, device=nodes.device).repeat(bsz, 1)
        initial_demands = (true_demands / capacity).unsqueeze(-1)
        
        # Используем первый state_encoder для получения представлений клиентов
        # Вход: координаты узлов, их индексы, депо как ориентир
        with torch.set_grad_enabled(self.training):
            initial_embeddings = self.state_encoders[0](
                nodes, all_nodes_idx, depot, depot, 
                true_demands/capacity, torch.ones((bsz, 1), device=nodes.device)
            )
